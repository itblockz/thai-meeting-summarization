"""
exp34 — exp30 with POOL_N=20 -> 30 (last E4 lever: pool size).

Only change vs exp30: dense top-30 ∪ BM25 top-30 instead of top-20 each
=> mean pool size grows from 28.57 to ~42.

Why try this — from eval_retrieval/pool_recall_eval.py on the same 1239
train queries:
    union_20 (exp30 baseline)  pool_size=28.57  pool_recall=0.9742  ref_recall=0.9066
    union_30 (this exp)        pool_size=42.22  pool_recall=0.9798  ref_recall=0.9288
                                                      (+0.0056)         (+0.0222)
The +2.2% ref_recall is the only retrieval-side lever that has NOT been
tested in the LLM pipeline. Going union_20 -> union_30 brings ~3.5
additional gold paragraphs (across the 1218 leak-free queries) into the
LLM context that exp30 would otherwise miss entirely (the 9.9%
"gold-not-in-pool" failure class shrinks).

Risks:
  * Saturation curve is bending hard (K=15->20 +0.0043, K=20->28(full)
    +0.0006); the new paragraphs at ranks 21-30 of dense/BM25 are by
    construction lower-quality, so they get low rerank scores and sit at
    the tail of the LLM context where attention is weak.
  * LLM context grows ~50% (mean 28 -> 42 paras, ~4.2K -> ~6.3K context
    tokens), well under max_model_len=16384 — no truncation, but
    runtime per query rises ~30-50%.
  * Historical note: POOL_N=20 -> 40 dropped rerank hit@1 by 0.0008 —
    rerank misranks the noisier candidates. exp34 sits at the
    midpoint (30), trading some of that risk for the recall gain.

Everything else identical to exp30:
  bge-reranker-v2-m3, full reranked pool (no GEN_K cap), E5 self-cite,
  exp08 few-shot, Qwen3-32B-AWQ, max_model_len=16384, enforce_eager.
"""
...
POOL_N         = 30     # CHANGED from 20 -> 30 (the only diff vs exp30)
EMBED_BATCH    = 64
MAX_NEW_TOKENS = 512
...
def main():
    ...
    n = len(queries)
    print(f"exp34: {n} queries, {len(doc_index)} docs "
          f"(POOL_N={POOL_N}, full pool, rerank ordered, no GEN_K cap)", flush=True)
