"""
exp27 — exp22 + GEN_K=20 (far end of K curve, = POOL_N).

GEN_K=20 equals POOL_N, so we're feeding the LLM essentially every
reranked candidate the stage-1 retrievers produced.

Pool coverage (bge-ranked):
  GEN_K=5  → 52.4% gold | hit 91.1% | oracle IoU 0.795
  GEN_K=10 → 61.1% gold | hit 95.1% | oracle IoU 0.851   (= exp25)
  GEN_K=15 → 67.0% gold | hit 96.3% | oracle IoU 0.880   (= exp26)
  GEN_K=20 → 70.8% gold | hit 97.1% | oracle IoU 0.899   (= this exp)

Pool size note: candidate pool from dense∪bm25 averages 28.5 paragraphs;
K=20 still leaves ~8 unconsidered. Recall headroom from K=15 is small
(+3.8pp gold) and each extra position is mostly non-gold ⇒ FP risk grows.

Cost: prompt ctx ~6488 chars (~2163 tokens, 13.2% of max_model_len);
LLM generation expected ~45-50 min.

Few-shot prompts still show 5 paragraphs (format generalizes); real query
gets 20. Everything else identical to exp22.
"""
...
GEN_K          = 20     # reranked paragraphs fed to the LLM as numbered context (vs exp22=5, exp25=10, exp26=15)
EMBED_BATCH    = 64
MAX_NEW_TOKENS = 512
...
def main():
    ...
    n = len(queries)
    print(f"exp27: {n} queries, {len(doc_index)} docs (GEN_K={GEN_K})", flush=True)
