"""
exp85 — S1=gemma → S2=A3B (hinted by gemma's answer+ref); score ALL 4 combos.

Model-swapped twin of exp84 (opposite hint direction):
Stage 1: gemma-4-26B-A4B-it-FP8-Dynamic does NORMAL V10 (answer + cite) on the
         full doc → (S1 answer, S1 refs).
Stage 2: A3B-Instruct-2507-FP8 does NORMAL V10 on the full doc, its prompt
         carrying gemma's answer ("คำตอบเบื้องต้น") AND gemma's ref indices
         ("ย่อหน้าที่เกี่ยวข้องเบื้องต้น: [X, Y]") → (S2 answer, S2 refs).

Emits ALL FOUR answer×ref combos and scores each (full + leak-free):
  s1ans_s1ref | s1ans_s2ref | s2ans_s1ref | s2ans_s2ref
Here S1=gemma (record citation IoU 0.8139), S2=A3B (strong answers). So
e.g. s2ans_s1ref = A3B's answer (hinted) + gemma's refs — the "best of both"
candidate, the mirror of exp84's s1ans_s2ref. Comparing exp84 vs exp85 isolates
whether it's better to (a) cite cold then write hinted, or (b) write cold then
cite hinted.
"""
...
# swapped vs exp84: gemma is Stage 1, A3B is Stage 2
...
def main():
    ...
    n = len(queries)
    print(f"exp85: S1={MODEL_STAGE1} → S2={MODEL_STAGE2} (hint=answer+ref); "
