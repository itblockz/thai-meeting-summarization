"""
exp24 — exp22 (E5 + bge reranker + exp08 few-shot) with POOL_N=20 → 40.

Motivation: the candidate pool (dense ∪ BM25 top-N) at POOL_N=20 covers
73.4% of gold refs (leak-free 1218). Widening to POOL_N=40 covers 84.3%
(+10.9pp). The gain concentrates on multi-ref queries:
  |gold|=1 : 98.3% → 99.1%  (saturated)
  |gold|=3 : 77.4% → 86.4%  (+9.0)
  |gold|=4 : 69.0% → 84.7%  (+15.7)
  |gold|≥6 : 45.6% → 66.9%  (+21.3)   ← biggest help

CLAUDE.md "What does NOT help" records a previous POOL_N=40 test that
lost hit@1 by −0.0008 on bge rerank alone — but that was the retrieval
metric only. E5 (LLM cites from top-5 of rerank) may still gain if the
extra gold bubbles into top-5.

Everything else is exp22 identical:
  - bge-reranker-v2-m3, GEN_K=5
...
Cost: rerank scores ~2× more pairs (avg 56 vs 28 per query).
"""
...
POOL_N         = 40     # top-N from each stage-1 retriever before rerank (vs exp22=20)
...
def main():
    ...
    n = len(queries)
    print(f"exp24: {n} queries, {len(doc_index)} docs (POOL_N={POOL_N})", flush=True)
