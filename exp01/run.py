"""
exp01 — textsum baseline pipeline + cross-encoder reranking.

Single change vs the textsum baseline: the retrieval stage is now a
two-stage retrieve-then-rerank — dense (bge-m3) + BM25 build a candidate
pool, a cross-encoder (bge-reranker-v2-m3) re-scores it, top-1 is kept.
Everything else (Qwen2.5-7B, prompt, TOP_K=1) is unchanged, so the score
delta is attributable to reranking alone.
"""
...
def main():
    ...
    n = len(queries)
    print(f"exp01: {n} queries, {len(doc_index)} docs", flush=True)
