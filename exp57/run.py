"""
exp57 — Pipeline 3 (hint, refs FREE): two-stage LLM. Stage A picks
"suggestion" refs; Stage B writes BOTH answer AND its own refs on FULL
context with a hint pointing at Stage A's refs.

Stage A: load Qwen3.6-27B-FP8, V10_factual + exp38 shots, full doc →
         per-query "suggested refs". USED ONLY AS HINT — discarded for
         the final submission.
Stage B: free 27B, load Qwen3-32B-AWQ. For each query, FULL doc context
         + "เน้นย่อหน้า [X, Y, Z] เป็นข้อมูลหลัก" hint. exp38 emits both
         answer AND citations. FINAL refs = Stage B's citations (may
         differ from Stage A).

Tests whether showing exp50's hint nudges exp38 toward higher IoU while
...
def main():
    ...
    n = len(queries)
    print(f"exp57 (hint, refs free): {n} queries, {len(doc_index)} docs", flush=True)
