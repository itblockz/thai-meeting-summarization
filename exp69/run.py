"""
exp69 — exp51 + FULL-DOC few-shot, 1-shot (single-ref Q0745).

Sibling of exp68 (1-shot multi-ref Q0746). Same two changes vs exp51
(A3B-Instruct + V10_factual, 0.7110 leak-free) — full-doc shot context +
1-shot — but keeps the SINGLE-ref example instead of the multi-ref one:

  Q0745 ('where was meeting 49 held') → gold ref P6, a single paragraph.
  This represents the 71.8% single-ref majority of the dataset prior.

exp68/exp69/exp70 form a clean few-shot ablation on top of the full-doc
realism change:
  exp68 — 1-shot, multi-ref (Q0746)
  exp69 — 1-shot, single-ref (Q0745)   ← this file
  exp70 — 2-shot, both

  1. **Full-doc shot context.** The shot shows the entire doc_050 (198
     valid paras, ~22.9K tokens) numbered [1..198] exactly like real
     inference; the [อ้างอิง: …] tag is recomputed to the gold ref's REAL
     position in the full doc (P6 → its filtered index), so the
     demonstration matches the actual task shape rather than a 5-para
     snippet with tiny indices.

Cost identical to exp68: worst-case prompt = shot(22.9K) + largest real
doc(27.6K) ≈ 50.5K → max_model_len 32768 → 57344, single A100-40GB,
prefix caching shares the 22.9K shot prefix.

Leak-free: shot is doc_050, held out from scoring (same as exp51).

Decision rule: accept if leak-free composite >= exp51 (0.7110).
Hypothesis: a single-ref full-doc demonstration may bias toward the
single-ref majority (higher precision on the 71.8%), trading off the
multi-ref recall that exp68's example teaches — the exp68 vs exp69
delta isolates which shot type the full-doc format prefers.
"""
...
# exp69 = 1-shot: single-ref only.
SHOTS = [_SHOT_SINGLEREF]
...
def main():
    ...
    print(f"exp69: {n} queries, {len(doc_index)} docs "
          f"(1-shot single-ref FULL-DOC few-shot from {SHOT_DOC_ID} "
