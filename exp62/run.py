"""
exp62 — exp42 full train eval with prompt_lab V14_factual_named_entities.

Model, shots, and decoding params are unchanged from exp42:
Qwen/Qwen3-30B-A3B-Instruct-2507-FP8, two exp38/39 shots, temperature=0.
Only the user prompt wording is changed to prompt_lab V14_factual_named_entities.

Decision rule: compare full and leak-free train metrics against exp42 to see
whether the R3 V14 prompt improvement on the 100-query prompt lab transfers to
the full 1239-query train evaluation.
"""
...
def main():
    ...
    n = len(queries)
    print(f"exp62: {n} queries, {len(doc_index)} docs "
    ...
    # Qwen3-30B-A3B-Instruct-2507-FP8 fits a single A100-40GB. vLLM reads
    ...
    # reserving multimodal encoder cache. gpu_memory_utilization=0.95 because
    # the 40 GB GPU is tight after the FP8 weights and KV cache.
