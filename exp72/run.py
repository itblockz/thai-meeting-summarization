"""
exp72 — model swap: A3B-Instruct-FP8 → RedHatAI/gemma-4-31B-it-FP8-block
(single A100-40GB). Sister run to exp71; identical pipeline, only the FP8
quant scheme differs (block-wise vs dynamic). Port of exp51 (A3B +
V10_factual prompt + exp38 shots, leak-free 0.7110).

FP8-block quantises weights AND activations in 128-wide blocks (DeepSeek-
V3 style). It is most efficient with block-scaled DeepGEMM kernels, which
need Hopper (H100); A100 has no native FP8 cores so vLLM falls back to a
generic dequant path — block scaling may be slower and less stable here
than exp71's per-channel/per-token dynamic scheme.

⚠️ TWO KNOWN RISKS specific to this variant:
  1. vllm#39407 — gemma-4 FP8_BLOCK produces garbage/repetitive output:
     absorbed activation scales get double-applied → logit saturation at
     the attention softcap. May be unpatched in 0.19.1.
  2. Block-scaled FP8 on A100 is off the optimised path (Hopper-targeted).
This run is mainly to confirm/deny whether block-FP8 is usable at all on
our hardware; exp71 (dynamic) is the one expected to be production-viable.
The first-run raw-output dump below is the gibberish tripwire.

Compatibility notes verified on LANTA (vllm 0.19.1, transformers 5.8.1):
- Gemma4ForConditionalGeneration is a registered arch.
- gemma-4's chat template supports `system` + accepts enable_thinking=False
  → exp51's system + 2-shot multi-turn message structure reused unchanged.
- limit_mm_per_prompt={"image":0,"video":0} skips the vision encoder cache.
...
MODEL_NAME     = os.environ.get("LLM_MODEL", "RedHatAI/gemma-4-31B-it-FP8-block")
...
def main():
    ...
    n = len(queries)
    print(f"exp72: {n} queries, {len(doc_index)} docs "
    ...
    # KV at 32K (needs 9.54 GiB, only ~5.7 free) → kv_cache_dtype="fp8"
    # halves KV (~4.77 GiB) + util 0.97. max_model_len stays 32768 so the
    # largest docs (~28K tok) are not truncated. See exp71 for rationale
    # (memory:prefer-single-gpu — adjust, don't TP=2).
