"""
exp26 — exp22 + GEN_K=15 (extending exp25's K=10 win further).

exp25 (GEN_K=10) lifted composite +0.0070 over exp22 (0.6647 → 0.6717),
with all 3 metrics rising in sync. This experiment tests whether the gain
saturates at K=10 or keeps climbing.

Pool coverage (bge-ranked):
  GEN_K=5  → 52.4% gold | hit 91.1% | oracle IoU 0.795
  GEN_K=10 → 61.1% gold | hit 95.1% | oracle IoU 0.851   (= exp25 ceiling)
  GEN_K=15 → 67.0% gold | hit 96.3% | oracle IoU 0.880   (= this exp)
  GEN_K=20 → 70.8% gold | hit 97.1% | oracle IoU 0.899

Diminishing returns: K=10→15 adds +5.9pp gold (vs +8.7pp from K=5→10).

Cost: prompt ctx ~5067 chars (~1689 tokens, 10.3% of max_model_len);
LLM generation expected ~35-40 min (vs exp25's 30 min).

Few-shot prompts still show 5 paragraphs (format generalizes); real query
gets 15. Everything else identical to exp22.
"""
...
GEN_K          = 15     # reranked paragraphs fed to the LLM as numbered context (vs exp22=5, exp25=10)
EMBED_BATCH    = 64
MAX_NEW_TOKENS = 512
...
def main():
    ...
    n = len(queries)
    print(f"exp26: {n} queries, {len(doc_index)} docs (GEN_K={GEN_K})", flush=True)
