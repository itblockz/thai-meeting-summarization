"""
exp53 — AWQ + V15_factual_no_redundant prompt (port from prompt_lab Round 3 — NEW AWQ leader).

Single-variable change from exp38 (AWQ best 0.6987 leak-free): swap the
"สรุปกระชับและครอบคลุม" wording to V15 which combines V10's factual
constraint with an explicit "ห้ามอ้างย่อหน้าที่ไม่ได้นำเนื้อหามาใช้"
guard. Model (Qwen3-32B-AWQ), shots (single-ref + Q0746 multi-ref),
decoding, GEN_K (no cap) all unchanged.

Why: V15 is the new AWQ leader in prompt_lab (100-query dev): proxy
0.6955 — beat V6 (0.6926) and V10 (0.6900). Both RougeL 0.4961 and IoU
0.6969 are competitive (V14 had higher RougeL but lower IoU; V6 had
higher IoU but lower RougeL). The "no redundant cite" guard sharpens
ref selection without hurting answer quality.

Decision rule: accept if leak-free composite >= exp38 (0.6987).
Realistic upside: composite 0.70+.
...
def main():
    ...
    n = len(queries)
    print(f"exp53: {n} queries, {len(doc_index)} docs "
