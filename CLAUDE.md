...
| exp80 † | exp59 pipeline + Stage A ref-picker → **gemma-4-26B-A4B-FP8** (refs fixed; IoU 0.8074 > 27B-FP8's 0.8006 but composite flat) | 0.7191 |
| exp81 † | exp60 pipeline + Stage A ref-picker → **gemma-4-26B-A4B-FP8** (refs free, A3B re-picks; best hybrid IoU 0.8135) | 0.7188 |
...
- **gemma-4-26B-A4B-FP8 as the hybrid Stage-A ref-picker** (exp80/exp81): **a wash, doesn't beat 27B-FP8.** Swapping exp59/exp60's Stage A (Qwen3.6-27B-FP8) for the exp74 gemma MoE — the bet being its record single-stage citation (IoU 0.8139) would sharpen refs without paying its answer-quality penalty (Stage B stays A3B). It DID nudge refs up — exp80 (refs fixed to Stage A) IoU **0.8074** vs exp59's 0.8006 (+0.0068); exp81 (refs free, A3B re-picks from gemma's soft hint) hit the best hybrid IoU **0.8135** — but composite stayed flat: exp80 **0.7191** (−0.0005 vs exp59 0.7196), exp81 **0.7188**. The +0.0068 IoU × 0.20 ≈ +0.0014 was cancelled by a small SS dip (exp80 SS 0.8614 / exp81 0.8577). The single-stage gemma IoU edge (0.8139) doesn't transfer to Stage A because there it competes with the *already-strong* 27B-FP8 (0.8006), not the weaker A3B self-cite — both dense extractors hit the same ~0.80–0.81 Stage-A ceiling. *Infra all clean* (both ran end-to-end, empty_answers=0, exp81 100% tag rate; gemma at util 0.95 + prefix caching as Stage A, A3B at 0.90 as Stage B, separate loads via del/gc/empty_cache; ~11–12 min/job). Keep 27B-FP8 as the hybrid ref-picker (exp56 0.7215 with 32B-AWQ Stage B remains best).
