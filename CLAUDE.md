...
| exp87 † | exp37 recipe + model → **Qwen3.6-27B-FP8** (single-stage; RougeL 0.4558 / SS 0.8480 / **IoU 0.7771** — sharpest single-stage citation here, but weakest answer) | 0.6966 |
| exp88 † | exp37 recipe + model → **Qwen3-30B-A3B-Instruct-2507-FP8** (single-stage; RougeL 0.4711 / SS 0.8607 / IoU 0.7512) — isolates the exp42 lift: model vs recipe | 0.7025 |
| exp89 † | exp51 pipeline + model → **nvidia/Qwen3-32B-NVFP4** (dense, pure-NVFP4 ModelOpt — FITS A100, attn quantized; RougeL 0.4882 / SS 0.8526 / **IoU 0.6951**) — strong answer but weak Qwen citation sinks it | 0.6936 |
| exp90 † | exp89 + publisher swap → **RedHatAI/Qwen3-32B-NVFP4** (dense, compressed-tensors `nvfp4-pack-quantized`, no override dir; RougeL 0.4884 / SS 0.8531 / **IoU 0.7013**) — publisher wash on dense Qwen3-32B | 0.6951 |
| exp91 † | exp73 + **enable_thinking=True** (gemma-4-31B-NVFP4 reasoning ON; RougeL 0.4680 / SS 0.8517 / **IoU 0.7802**) — thinking disrupts gemma's native citation+phrasing, −0.0109 vs exp73 | 0.7031 |
...
- **Single-stage model sweep on the exp37 recipe (exp37/87/88):** holding exp37's recipe fixed (context-first, single-ref shots, 512 tok) and swapping only the model: A3B (exp88, **0.7025**) > 27B-FP8 (exp87, **0.6966**) > 32B-AWQ (exp37, 0.6944). 27B-FP8 has the **sharpest single-stage citation** (IoU 0.7771, 16/1239 fallbacks) but the **weakest answers** (RougeL 0.4558 / SS 0.8480) — the 0.45+0.35 answer weight sinks it below A3B, consistent with 27B-FP8 being a ref-picker (exp56 Stage-A) not an answer-writer. exp88 also **decomposes exp42's lift over exp37** (0.7087 vs 0.6944): A3B-the-model gives +0.0081 (exp88), and the rest (−0.0062 gap exp88→exp42) is the exp38 recipe (1024 tok + multi-ref shot2). Recipe and model both matter; neither alone reaches exp42.
...
- **nvidia/Qwen3-32B-NVFP4 as a single-model swap on the exp51 line** (exp89): **runs/fits, but loses — 0.6936 leak-free, −0.0204 under dense gemma-4-31B-NVFP4 (exp73 0.7140), the entire gap is IoU.** This nvidia/ModelOpt build is the FEASIBLE NVFP4 case (contrast the infeasible walls): `hf_quant_config` = pure `quant_algo:"NVFP4"` (NOT `modelopt_mixed` → cleared the exp78 sm89 gate) and `exclude_modules:["lm_head"]` only → **attention IS quantized** (unlike the dense gemma-4-31B ModelOpt build exp75 that kept all self_attn bf16 → 30 GiB → didn't fit). ~18 GiB load + Qwen3's cheap GQA KV (8 heads, no gemma sliding-window floor) → 62,768-tok KV / **full 32768 ctx, no truncation** on a single A100-40GB. The override-dir strip of `kv_cache_quant_algo`/`kv_cache_scheme` (exp77 route) was still needed (KV → bf16). Breakdown: RougeL **0.4882** / SS 0.8526 / **IoU 0.6951** — **answer quality is competitive with the dense gemma** (RougeL even higher than exp73's 0.4790; SS ~−0.002), but the **single-stage citation is the wall**: only 1142/1239 emitted a tag (97 fallbacks), avg **1.34 refs/query** → IoU 0.6951 vs gemma-4's 0.8091 (−0.114 × 0.20 = −0.0228, ≈ the whole composite gap). NVFP4 vs AWQ on the *same* Qwen3-32B base is citation-neutral (IoU 0.6951 ≈ exp38 AWQ 0.6906) — **the quant didn't move it; the model's instruction-following did.** Confirms the CLAUDE.md priors: **gemma-4 follows the citation instruction near-perfectly (99.7% tag rate) and Qwen does not** — so Qwen3-32B is a strong *answer-writer* (cf. its 32B-AWQ sibling as exp86's Stage-B) but a poor single-stage *ref-picker*. Dense gemma-4-31B-NVFP4 (exp73) stays the best single model.
- **RedHatAI/Qwen3-32B-NVFP4 vs nvidia/ModelOpt — publisher swap is a WASH on dense Qwen3-32B** (exp90): 0.6951 leak-free vs exp89's 0.6936 = **+0.0015 (noise)**, RougeL 0.4884 / SS 0.8531 / IoU 0.7013. RedHatAI ships compressed-tensors `nvfp4-pack-quantized` (`config_groups targets:['Linear']`, `kv_cache_scheme:None`, no `hf_quant_config.json`) → pure-NVFP4, runs on sm80 via MARLIN with **NO override dir** (contrast exp89's ModelOpt build that bakes a FP8-KV directive needing the strip). UNLIKE the dense gemma-4-31B (where RedHatAI's quantized attn was the *only* feasible build, exp73 vs exp75-infeasible), here **both publishers FIT** — Qwen3-32B is text-only with cheap GQA KV, AND exp89's nvidia/ModelOpt build already quantizes attention (`exclude_modules:["lm_head"]` only, not gemma's all-self_attn exclusion) → ~18 GiB either way. So with feasibility off the table, the publisher choice is a pure answer-quality wash on this dense model (RedHatAI edges it by IoU +0.006, within noise). The dense "RedHatAI > ModelOpt" rule was a *feasibility* rule (gemma), not a *quality* rule — when ModelOpt also quantizes attention and fits, they tie. Either way Qwen3-32B single-stage caps ~0.695 (cf. AWQ exp37 0.6944) — the weak-citation ceiling (IoU ~0.70) holds across publisher AND quant; dense gemma-4-31B-NVFP4 (exp73 0.7140) stays the best single model.
- **enable_thinking=True on gemma-4-31B-NVFP4** (exp91): **thinking-ON HURTS — 0.7031 leak-free, −0.0109 under exp73's thinking-OFF 0.7140.** Single-variable flip of exp73 (RedHatAI gemma-4-31B-NVFP4, V10 prompt, exp38 2-shot, full-doc) to reasoning mode via `apply_chat_template(enable_thinking=True)`, MAX_NEW_TOKENS 1024→4096. Run was clean (1211/1239 citation tags, 1.88 refs/q, 0/1239 truncated by MAX_NEW_TOKENS) so the result is a real quality signal, not a strip bug. Breakdown vs exp73: RougeL 0.4680 (−0.0110), SS 0.8517 (−0.0029), **IoU 0.7802 (−0.0289 — the dominant loss)**. Thinking disrupts gemma-4's two *native* strengths: its near-perfect one-pass citation (the reasoning step makes it cite a broader/different set → IoU precision drops) and its faithful phrasing (reason-then-answer drifts the final wording from gold → RougeL drops); SS barely moves. So gemma is best left as a cold one-pass ref-picker/answerer — reasoning only adds noise. **Two gotchas confirmed for any future gemma thinking run** (the strip plumbing itself works): (1) gemma-4's reasoning channel is delimited `<|channel>thought…<channel|>` (mirrored brackets, verified from `chat_template.jinja`), NOT Qwen's `<think>…</think>`; (2) those are SPECIAL tokens — vLLM's default `skip_special_tokens=True` deletes them before `output.text`, leaving naked chain-of-thought that leaks into the answer (exp91 run-1 = 0.5437, RougeL collapsed to 0.1297); fix is `SamplingParams(skip_special_tokens=False)` + regex-strip the closed block, the truncated/unclosed block, and leftover special tokens before citation parsing. Verdict: keep `enable_thinking=False`; exp73 stays the best single model. (No untried lever remains on the gemma single-model axis.)
