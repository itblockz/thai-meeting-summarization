...
| `exp68/` — full-doc few-shot 1-shot (A3B) | exp51 + multi-ref shot (Q0746) shown over the WHOLE doc_050 | 0.7067 † |
| `exp69/` — full-doc few-shot 1-shot (A3B) | exp51 + single-ref shot (Q0745) over the whole doc_050 | 0.7087 † |
| `exp70/` — full-doc few-shot 2-shot (A3B) | exp51 + both shots over the whole doc_050 (apples-to-apples) | 0.7114 † |
...
**Full-doc few-shot (exp68–exp70, leak-free) — NEUTRAL, dead-end**: exp51's few-shot shows each example over a hand-picked 5-paragraph snippet of doc_050 (citation indices 2..5 — tiny, unlike the ~180-para real doc). exp68–70 make the demonstration realistic: show the ENTIRE doc_050 (198 valid paras, ~22.9K tok) numbered [1..198] like real inference, with the [อ้างอิง: …] tag recomputed to the gold refs' REAL positions (P6→.., P21–24→..). Few-shot composition ablated on top: exp68 = 1-shot multi-ref (0.7067), exp69 = 1-shot single-ref (0.7087), exp70 = 2-shot both (0.7114). **The apples-to-apples test is exp70 (2-shot full-doc) vs exp51 (2-shot snippet): 0.7114 vs 0.7110 = +0.0004, pure greedy-decode noise** — IoU identical too (0.7758 vs 0.7754). So realistic full-doc citation indices teach the model **nothing extra** over the cheap snippet; the model already generalises the [อ้างอิง:] format from small examples. Secondary: 2-shot still beats 1-shot (exp70 > exp68/exp69), and within 1-shot single-ref > multi-ref (exp69 0.7087 > exp68 0.7067 — the multi-ref-only shot over-cites without a single-ref anchor). **Verdict: keep exp51's snippet few-shot — full-doc costs ~2.3× the prompt tokens (doc_050 is the 2nd-largest doc, 22.9K; 2 copies + the largest real doc = ~74K, OOMs a single A100-40GB at default util — exp70 needed max_model_len trimmed to 77824 + gpu_mem_util 0.97 to fit one GPU) for zero gain.**
