"""
exp15 — exp08 with criteria-selected example pair.

exp08's hand-picked Q0745+Q0747 produced the best gain (+0.0091) by
luck — analysis showed 4 structural factors driving the gain (restate
pattern strength, length proximity to gold median, query-type
frequency, centrality). exp08's Q0745 covers only the rare "ที่ใด"
location pattern (1.8% of dataset).

This run swaps Q0745 → Q0758: an explanation-type ("ทำไม") example
that covers the most frequent query pattern (44.5% of dataset),
keeps a strong restate (0.60), and has 48-token gold (closer to
ideal 50-tok length). Q0747 retained (top-1 by criteria, score 0.710).
Pair criteria-score 1.369 vs exp08's 1.271 — single-variable test
of "explicit criteria > hand-pick" hypothesis.

Held-out doc unchanged: doc_050 (same as exp06, exp08, exp09–11).
"""
...
# Criteria-selected pair from doc_050 (Q0747 + Q0758). Q0747 was kept from
# exp08 (top-1 by criteria); Q0758 replaces Q0745, contributing the high-frequency
# "ทำไม" explanation pattern that exp08 lacked.
FEW_SHOT = [
    {
    ...
    {
        "query": "ทำไมบริษัทประกันสินเชื่ออุตสาหกรรมขนาดย่อม (บสย.) จึงมีการจัดทำมาตรการสีฟ้าเพิ่มขึ้นมา",
        "paragraph": (
            "ในปี พ.ศ. ๒๕๖๗ บสย. ได้มีการปรับปรุงมาตรการ ๓ สี "
            "(ม่วง เหลือง เขียว) โดยเพิ่มเติมมาตรการสีฟ้า "
            "เพื่อให้ความช่วยเหลือลูกหนี้จากมาตรการสีเขียวที่ผ่อนชำระหนี้"
            "ปกติต่อเนื่องมาแล้ว ๓ งวด สามารถเข้าร่วมมาตรการสีฟ้า "
            "เพื่อปลดหนี้ ลดต้นร้อยละ ๑๕ ได้"
        ),
        "answer": (
            "สาเหตุที่บริษัทประกันสินเชื่ออุตสาหกรรมขนาดย่อมมีการทำ"
            "มาตรการสีฟ้าเพิ่มขึ้นมาจากปกติที่มี 3 สี เนื่องจากว่า"
            "มาตรการสีฟ้านี้จะสามารถให้ลูกหนี้ของมาตรการสีเขียวที่ผ่อนชำระหนี้ "
            "เข้ามาร่วมมาตรการนี้ เพื่อที่จะปลดหนี้ และลดเงินต้นได้ ๑๕%"
        ),
    },
]
