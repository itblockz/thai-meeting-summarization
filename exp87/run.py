"""
exp87 — exp37 base, model swap Qwen3-32B-AWQ → Qwen3.6-27B-FP8.
...
the generation model to **Qwen/Qwen3.6-27B-FP8** (dense 27B, FP8-e4m3).

Motivation: exp37 is the 32B-AWQ single-stage line. exp42 already moved
A3B onto the (later) exp38/39 recipe (1024 tokens, multi-ref shot2), but
27B-FP8 was only ever run single-stage on the V10_factual ref-picker
recipe (exp50) and as a hybrid Stage-A (exp56). This isolates 27B-FP8 on
the *clean exp37 answer recipe* for an apples-to-apples model comparison
against exp37 (32B-AWQ) and exp88 (A3B).

Model load copied from exp63 (the 27B-FP8 + enable_prefix_caching=True
fix — exp63 found prefix caching is output-neutral and ~2.9x faster; 27B
resolves to a multimodal arch that auto-disables it, so force it on):
  dtype=bfloat16, gpu_memory_utilization=0.95, trust_remote_code=True,
  limit_mm_per_prompt={"image":0,"video":0}, enable_prefix_caching=True.
A100 has no native FP8 cores → vLLM dequantises FP8→FP16 via fp8_marlin.

Decision rule: compare leak-free composite against exp37 (0.6944) and
exp88 (A3B on the same recipe). 27B is dense → expect quality per param
to edge A3B's ~3B-active subset; the open question is vs 32B-AWQ.
"""
...
def main():
    ...
    n = len(queries)
    print(f"exp87: {n} queries, {len(doc_index)} docs "
    ...
    # Qwen3.6-27B-FP8: dense 27B FP8-e4m3 on a single A100-40GB (config from
    # exp63). vLLM reads quant_method=fp8 from config.json → fp8_marlin /
    # fp8_w8a16 kernel on A100 (no native FP8 cores → dequant FP8→FP16).
    # Vision blocks load but stay idle; limit_mm_per_prompt skips the encoder
    # cache budget. The 27B resolves to a multimodal arch that auto-disables
    # prefix caching → force enable_prefix_caching=True (exp63: output-neutral,
    # ~2.9x faster on the shared full-doc prefix).
