"""
exp06 — exp03 retrieval pipeline + 2-shot prompting from held-out doc_050.

Single change vs exp03: the LLM prompt now includes 2 worked examples
(query, paragraph, gold-answer) drawn from doc_050. doc_050's 21 queries
are excluded from inference to avoid leakage — comparison must use the
exp03 baseline rescored on the same 1218-query subset.

Few-shot examples come from doc_050 because it is the last document in
the train set; this is a fixed, deterministic held-out split that is
trivial to re-derive at inference time on a different dataset.
"""
...
HELDOUT_DOC = "doc_050"   # source of few-shot examples; excluded from inference
...
# Two worked examples from doc_050. Both are single-ref (TOP_K=1 pattern):
# Q0745 — short factoid answer; Q0747 — medium reason/purpose answer.
FEW_SHOT = [
    {
    ...
    },
    {
        ...
        "paragraph": (
            ...
            "ด้านการประชุม การศึกษาดูงาน และการจัดสัมมนา เพื่อนำผลการประเมิน"
            "ความพึงพอใจและความไม่พึงพอใจมาเป็นข้อมูลในการทบทวน ปรับปรุง "
            "และพัฒนาการปฏิบัติงานให้มีประสิทธิภาพต่อไป"
        ),
        "answer": (
            "การจัดทำแบบสำรวจความพึงพอใจและไม่พึงพอใจของคณะกรรมการในครั้งนี้ "
            "มีการจัดทำขึ้นเพื่อนำข้อมูลที่ได้มาทบทวน ปรับปรุง รวมถึงนำไปพัฒนา"
            "การปฏิบัติงานให้มีประสิทธิภาพยิ่งขึ้น"
        ),
    },
]
