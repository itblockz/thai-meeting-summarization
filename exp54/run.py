"""
exp54 — Qwen3.6-27B-FP8 + V13_factual_extract prompt (IoU ceiling test).

Single-variable change from exp40 (0.6969 leak-free): swap to V13
"สั้นและตรงประเด็น + ใช้ถ้อยคำเดิมจากย่อหน้าให้มากที่สุด ห้ามเรียบเรียงใหม่".
Model, shots, decoding all unchanged.

Why: prompt_lab dev (100 leak-free queries) — V13 on Qwen3.6-27B-FP8
gave **IoU 0.8225** (record across ALL variants × models tested) but
RougeL crashed to 0.3745 (model becomes ultra-terse copy-paste of
para fragments). The bet: at full 1218 the IoU advantage over exp40's
0.7719 may outweigh the RougeL/SS dip if SS-score (semantic similarity
of bge-m3 embeddings) is forgiving for extractive answers. RougeL on
the dev set proxy was 0.6781 (worst R3 27B variant) — risky.

This is a CEILING TEST: if it lands ≥ exp40 (0.6969), V13 is a viable
high-IoU prompt for 27B-FP8 family. If composite < 0.65, abandon
extractive approach on this model.

Original exp40 docstring below for context.

exp40 (base) — model swap: Qwen3-32B-AWQ → Qwen3.6-27B-FP8 (single A100-40GB).
...
def main():
    ...
    n = len(queries)
    print(f"exp54: {n} queries, {len(doc_index)} docs "
