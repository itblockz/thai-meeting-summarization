"""
exp21 — exp20 with the per-paragraph length cap removed (ladder step 3/3,
final = toey/exp01 reproduction).

Single change vs exp20: build_prompt no longer truncates paragraphs at
MAX_PARA_CHARS — the numbered context shows each paragraph in full, as
toey/exp01 does. With this, exp21 matches toey/exp01's E5 + few-shot
config in every output-affecting respect (retrieval, Qwen3-32B-AWQ,
GEN_K=5, prompt, the 2 doc_050 few-shot examples, citation parser,
sampling), so its composite should reproduce toey's 0.6522.

exp18 -> exp19 -> exp20 -> exp21 isolates few-shot, SYSTEM_MSG and the
paragraph cap, one variable per step. See exp19 for the full rationale.
"""
...
# toey/exp01's system message (exp19 used exp02/exp03's "สรุปจากข้อมูล").
SYSTEM_MSG = (
...
def main():
    ...
    n = len(queries)
    print(f"exp21: {n} queries, {len(doc_index)} docs", flush=True)
