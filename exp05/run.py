"""
exp05 — textsum rerank pipeline + Qwen3-32B-AWQ + direct-QA prompt.

Single change vs exp03: prompt is reframed from "summarize document" to
"answer question directly", and the model is told it can rephrase. The
target style is the gold abstractive — ~175 chars, 92% rephrased, direct
QA — so the old "summarize concisely AND comprehensively" instruction
(internally contradictory) was likely pushing the model off-target.
Retrieval + generation LLM are identical to exp03.
"""
...
SYSTEM_MSG = (
    "คุณเป็นผู้ช่วยตอบคำถามจากเอกสารภาษาไทย "
    "ตอบให้ตรงประเด็นและกระชับ อ้างข้อเท็จจริงจากเอกสารที่ให้มาเท่านั้น ห้ามแต่งเติม"
)
...
def build_prompt(query, retrieved_texts):
    ...
    return (
        ...
        f"คำสั่ง: ตอบคำถามให้ตรงประเด็นและกระชับ "
        f"โดยอ้างข้อเท็จจริงจากเอกสารที่ให้มา สามารถเรียบเรียงใหม่ได้\n"
        f"คำตอบ:"
    )
