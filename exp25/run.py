"""
exp25 — exp22 (E5 + bge reranker + exp08 few-shot) with GEN_K=5 → 10.

Motivation: at GEN_K=5, bge top-5 covers 55.7% of all gold refs and 91.1%
of queries (hit@5). Going to GEN_K=10:
  pool %gold        : 55.7% → 65.3%  (+9.6pp)
  hit@10            : 91.1% → 95.1%  (+4.0pp)
  IoU oracle ceiling: 0.816 → 0.876  (+0.060)

Per-|gold| bucket the gain concentrates on multi-ref queries:
  |gold|=1 : 94.6% → 97.2%  (saturated)
  |gold|=3 : 50.3% → 67.2%  (+16.9)
  |gold|=4 : 39.8% → 59.3%  (+19.5)
  |gold|≥6 : 19.7% → 31.3%  (+11.6)

Risk: LLM cite rate at rank 4-5 in exp22 was already low precision
(38-31%); ranks 6-10 likely lower → more FP per cite. Net IoU depends
on whether the +9.6pp recall headroom outweighs the precision drag.

Few-shot prompts still show 5 paragraphs (the format generalizes);
only the real query gets 10. Cost: prompt tokens 679 → 1197 (still 14.6%
of max_model_len=16384). Everything else identical to exp22.
"""
...
GEN_K          = 10     # reranked paragraphs fed to the LLM as numbered context (vs exp22=5)
EMBED_BATCH    = 64
MAX_NEW_TOKENS = 512
...
def main():
    ...
    n = len(queries)
    print(f"exp25: {n} queries, {len(doc_index)} docs (GEN_K={GEN_K})", flush=True)
