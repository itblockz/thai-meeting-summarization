...
`abstractive` answer to the first N Thai words (pythainlp newmm), for a
range of caps N, then recomputes the composite score for every cap.
...
# Word caps to sweep. None = uncapped baseline (exp51 as-is).
CAPS = [8, 12, 16, 20, 25, 30, 40, 50, 75, 100, None]

WSS, WRL, WJ = 0.45, 0.35, 0.20


def cap_answer(text, n_words):
    """Truncate `text` to its first n_words newmm tokens, faithfully
    reconstructing spacing (whitespace tokens kept, joined with '')."""
    if n_words is None or not isinstance(text, str) or not text.strip():
        return text
...
def main():
    ...
    variants = {}  # cap -> list[str]
    for cap in CAPS:
        variants[cap] = [cap_answer(t, cap) for t in df["abstractive_pred"]]
    ...
    for cap in CAPS:
        ...
        rows.append({"cap": cap, "rougeL": rl, "SS": ss, "IoU": iou,
                     "composite": comp, "avg_len": avg_len})
        if cap is None:
            base_comp = comp
    for r in rows:
        delta = "" if base_comp is None else f"{r['composite']-base_comp:+.4f}"
        label = "∞" if r["cap"] is None else str(r["cap"])
        print(f"{label:>6}  {r['rougeL']:>7.4f}  {r['SS']:>7.4f}  "
              f"{r['IoU']:>7.4f}  {r['composite']:>9.4f}  {delta:>8}  "
              f"{r['avg_len']:>7.1f}")

    best = max(rows, key=lambda r: r["composite"])
    blabel = "∞" if best["cap"] is None else str(best["cap"])
    print(f"\nbest cap = {blabel}  composite = {best['composite']:.4f}")

    out = submission.parent / "cap_sweep.json"
    out.write_text(json.dumps(
        [{**r, "cap": (r["cap"] if r["cap"] is not None else "inf")} for r in rows],
