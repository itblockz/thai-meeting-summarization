"""
exp64 — Pipeline 1 (filter), Stage A swapped 27B-FP8 → A3B-Instruct.

Mirror of exp55 but the REF-PICKING model (Stage A) is swapped from
Qwen3.6-27B-FP8 to Qwen3-30B-A3B-Instruct-2507-FP8. Stage B stays
Qwen3-32B-AWQ (exp38's answer model).

Stage A: load A3B-Instruct, V10_factual + exp38's multi-ref shots, full
         doc context → fresh per-query refs.
Stage B: free A3B, load Qwen3-32B-AWQ. For each query, TRUNCATE the doc
         to only the paragraphs Stage A cited (preserve doc order,
         renumber 1..K), run with exp38's E5 prompt + shots → fresh
         per-query answer.

Final submission: refs = Stage A's fresh refs, abstractive = Stage B's
fresh answer. NO precomputed CSVs are read.

Hypothesis: A3B-Instruct follows the [อ้างอิง: …] format more reliably
than 27B-FP8 (exp42 lesson) and is ~4x faster wall-clock, so it may pick
refs at least as well for Stage A while removing the FP8-Marlin prefill
penalty. Risk: 27B's dense factual extraction (V10 IoU 0.7998 in exp50)
may not transfer to A3B's ~3B active MoE.
"""
...
def build_prompt_v10(query, paras):
    """V10_factual prompt (used in Stage A for refs)."""
...
def main():
    ...
    n = len(queries)
    print(f"exp64 (filter, Stage A=A3B): {n} queries, {len(doc_index)} docs", flush=True)

    doc_paras = {}
    for doc_id, paragraphs in doc_index.items():
        doc_paras[doc_id] = filter_valid_paragraphs(paragraphs)

    # ----- Stage A: A3B-Instruct picks refs on full context -----
