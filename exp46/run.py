"""
exp46 — MAX_NEW_TOKENS 512 → 1024 (rescue truncated long-answer queries).

Single-variable ablation of exp38 (NEW BEST 0.6987 leak-free): double
the generation cap so the small fraction of long-answer queries can
finish their answer and emit the [อ้างอิง: …] tag.

Motivation: exp38's 106 remaining fallback queries (no tag emitted →
refs = gen_pids[0]) skew toward long outputs:
  - 44/106 end mid-list (last 100c looks like a list-item continuation)
  - 33/106 have answer length >=500 chars (~350+ tokens)
  - several samples observed truncating mid-word ("เมื่อป...", "เช่...",
    "21 -..."), pointing at MAX_NEW_TOKENS=512 hitting the cap.

Gold token-count statistics (Qwen3 tokenizer on the train set, 1239
queries) — measured 2026-05-27:
  P50 = 95, P90 = 260, P95 = 360, P99 = 625, max = 1337 tokens.
With ~20 tokens of citation-tag overhead and the model's typical ~1.2x
verbosity vs gold, max_tokens=512 truncates roughly P98-P99 of queries
(~24/1239 gold-side); the observed verbose factor pushes the actual
model-side truncation rate higher, matching the 44 mid-list endings.

max_tokens=1024 covers P99.9 of gold (only 1 query needs more, the
max-1337-token outlier). vLLM does not pre-allocate KV by max_tokens,
so the extra budget costs only the tokens actually generated (a few
hundred queries × at most ~5 extra tokens vs current = negligible
wall-clock).

Decision rule: accept exp46 if leak-free composite >= exp38 (0.6987).
Expected: +0.004–0.008 composite, mostly IoU (fallbacks → real refs).
"""
...
MAX_NEW_TOKENS = 4096  # exp4x: thinking — block can be long
...
# Same shots as exp38: shot1 single-ref (exp08 carry), shot2 multi-ref
# (Q0746 from doc_050 — 4-of-5 subset).
...
def main():
    ...
    n = len(queries)
    print(f"exp46: {n} queries, {len(doc_index)} docs "
    ...
    for it, out in zip(items, outputs):
        ...
        # exp4x: strip <think>...</think> block(s) before citation parsing
