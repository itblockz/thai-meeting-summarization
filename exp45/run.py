"""
exp45 — model swap: Qwen3-32B-AWQ → Qwen3.6-27B-FP8 (single A100-40GB).

Multi-variable change from exp39 (prev best 0.6991): swap the LLM from
Qwen3-32B-AWQ (dense, AWQ-Marlin 4-bit) to **Qwen/Qwen3-30B-A3B-Thinking-2507-FP8**
...
already on via enable_thinking=True in apply_chat_template).
...
Decision rule: accept exp45 if leak-free composite >= exp39 (0.6991).
...
MAX_NEW_TOKENS = 4096  # think block can be long
MAX_MODEL_LEN  = int(os.environ.get("MAX_MODEL_LEN", "32768"))
TP_SIZE        = int(os.environ.get("TP_SIZE", "1"))
MODEL_NAME     = os.environ.get("LLM_MODEL", "Qwen/Qwen3-30B-A3B-Thinking-2507-FP8")
...
def main():
    ...
    n = len(queries)
    print(f"exp45: {n} queries, {len(doc_index)} docs "
    ...
    for it, out in zip(items, outputs):
        ...
        # exp45: strip <think>...</think> block(s) before citation parsing
