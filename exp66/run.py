"""
exp66 — Pipeline 3 (hint, refs FREE), Stage A swapped 27B-FP8 → A3B.

Mirror of exp57 but the SUGGESTION model (Stage A) is swapped from
Qwen3.6-27B-FP8 to Qwen3-30B-A3B-Instruct-2507-FP8. Stage B stays
Qwen3-32B-AWQ and emits BOTH the answer and the final refs.

Stage A: load A3B-Instruct, V10_factual + exp38 shots, full doc →
         per-query "suggested refs". USED ONLY AS HINT — discarded for
         the final submission.
Stage B: free A3B, load Qwen3-32B-AWQ. For each query, FULL doc context
         + "เน้นย่อหน้า [X, Y, Z] เป็นข้อมูลหลัก" hint. exp38 emits both
         answer AND citations. FINAL refs = Stage B's citations (may
         differ from Stage A).

Tests whether an A3B hint nudges exp38 toward higher IoU while
preserving exp38's stronger abstractive (RougeL/SS).
"""
...
def main():
    ...
    n = len(queries)
    print(f"exp66 (hint, refs free, Stage A=A3B): {n} queries, {len(doc_index)} docs", flush=True)

    doc_paras = {}
    for doc_id, paragraphs in doc_index.items():
        doc_paras[doc_id] = filter_valid_paragraphs(paragraphs)

    # ----- Stage A: A3B-Instruct → suggestion refs (hint only) -----
    print(f"\n=== Stage A: {MODEL_A3B} → suggestion refs (hint only) ===", flush=True)
    items_A = []
    ...
    # ----- Stage B -----
    print(f"\n=== Stage B: {MODEL_AWQ} → answer + own refs ===", flush=True)
    items_B = []
