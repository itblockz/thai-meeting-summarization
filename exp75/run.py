"""
exp75 — NVFP4 checkpoint swap: RedHatAI/gemma-4-31B-it-NVFP4 (exp73)
→ nvidia/Gemma-4-31B-IT-NVFP4 (single A100-40GB). Everything else is
identical to exp73 (exp51 prompt + V10_factual + exp38 shots, full doc).
Only the NVFP4 *publisher/format* changes.

WHY this is worth a run: exp73 (RedHatAI NVFP4) = 0.7140 leak-free, the
best single model. RedHatAI packs NVFP4 via compressed-tensors
(`nvfp4-pack-quantized`); NVIDIA's official checkpoint packs the SAME 4-bit
NVFP4 (E2M1 weights + fp8_e4m3 group scales) via TensorRT-Model-Optimizer
(`quant_method: modelopt`/ModelOpt FP4). vLLM has separate loaders for the
two (ModelOptNvFp4 vs compressed-tensors NvFp4), so per-layer scale
placement and any per-tensor amax differ slightly → a small output drift is
possible. If NVIDIA's calibration is tighter the answer quality (RougeL/SS)
could edge up at the same ~22 GB footprint; if not, expect ~exp73.
...
Decision rule: accept if leak-free composite >= exp73 (0.7140).
"""
...
def main():
    ...
    n = len(queries)
    print(f"exp75: {n} queries, {len(doc_index)} docs "
    ...
    # nvidia/Gemma-4-31B-IT-NVFP4: ModelOpt-packed NVFP4 (quant_method=
    # modelopt). On A100/sm80 there are no native FP4 tensor cores → vLLM
    # uses a Marlin FP4 weight-only dequant path (same fallback class exp73
