"""
exp20 — exp19 + toey/exp01's SYSTEM_MSG (step 2 of the E5 + few-shot ladder).

Single change vs exp19: the system message switches from exp02/exp03's
"...ตอบคำถามโดยสรุปจากข้อมูลที่ให้มาเท่านั้น..." to toey/exp01's
"...ตอบคำถามโดยอ้างอิงจากย่อหน้าที่ให้มาเท่านั้น...". Isolates the effect of
the system-prompt wording. Few-shot, GEN_K=5, max_model_len=16384 and the
MAX_PARA_CHARS=600 cap are all exp19's. exp21 removes the cap (= full
toey/exp01 reproduction).

See exp19 for the ladder rationale (exp17/exp18: E5 alone ~break-even vs
exp03; toey/exp01 E5 + few-shot = 0.6522).
"""
...
def main():
    ...
    n = len(queries)
    print(f"exp20: {n} queries, {len(doc_index)} docs", flush=True)
