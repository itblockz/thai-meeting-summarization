"""
exp71 — model swap: A3B-Instruct-FP8 → RedHatAI/gemma-4-31B-it-FP8-Dynamic
(single A100-40GB). Single-variable port of exp51 (A3B + V10_factual
prompt + exp38 shots, leak-free 0.7110). Only the LLM changes.

gemma-4-31B-A4B is a MoE (~25-31B total / ~4B active). The FP8-Dynamic
checkpoint quantises weights statically (per-channel) and activations
dynamically per-token at inference — data-free, no block-scaled DeepGEMM
kernel required, so it is the A100-safe FP8 variant (block-scaled FP8
needs Hopper; see exp72). On A100 (no native FP8 cores) vLLM dequantises
FP8→bf16 via fp8_marlin, same as the A3B/27B-FP8 runs.

Compatibility notes verified on LANTA (vllm 0.19.1, transformers 5.8.1):
- Gemma4ForConditionalGeneration is a registered arch.
- gemma-4's chat template DOES support the `system` role and accepts the
  `enable_thinking=False` kwarg (unlike gemma-2/3), so exp51's
  system + 2-shot multi-turn message structure is reused unchanged.
- gemma-4 has a thinking "channel"; enable_thinking=False suppresses it.
  ⚠️ FIRST-RUN CHECK: inspect raw output[0] in the .out — if any
  <|channel>…<turn|> thinking text leaks into `answer`, add a strip here.
- limit_mm_per_prompt={"image":0,"video":0} stops the vision tower from
  reserving encoder-cache budget (gemma-4 is multimodal).

Known risk: vllm#39049 reported gibberish from gemma-4 FP8 *dynamic*
quant — may be fixed in 0.19.1; the first-run output sanity check above
will catch it.
...
MODEL_NAME     = os.environ.get("LLM_MODEL", "RedHatAI/gemma-4-31B-it-FP8-Dynamic")
...
def main():
    ...
    n = len(queries)
    print(f"exp71: {n} queries, {len(doc_index)} docs "
    ...
    # weights leave only ~5.7 GiB for KV at util 0.95, but bf16 KV for the
    # 32K context needs 9.54 GiB → OOM. Fix per memory:prefer-single-gpu
    # (adjust, don't TP=2): kv_cache_dtype="fp8" halves KV (~4.77 GiB @ 32K)
    # and util 0.97 adds headroom. max_model_len stays 32768 so the largest
    # docs (~28K tok) are NOT truncated. NB: FP8 KV is a minor confound vs
    # exp51's bf16 KV, but unavoidable to keep full-doc context here.
